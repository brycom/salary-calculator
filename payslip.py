...
def payslip(employee):
    ...
    # employee info
    c.setFontSize(24)
    c.drawString(width / 8, height - inch / 1.5, employee.company)
    c.setFontSize(12)
    c.drawString(inch * 6, height - inch * 0.4, f"Name: {employee.name}")
    c.drawString(inch * 6, height - inch * 0.6, f"Nr: {employee.employment_number}")
    c.drawString(
        inch * 6,
        height - inch * 0.8,
    ...
    c.drawString(
        inch * 3, height - inch * 2.4, f"{employee.workhours - employee.overtime}h"
    )
    ...
    c.drawString(inch * 5, height - inch * 2.6, f"{employee.total_overtime_pay}kr")
    # sickleave
    sick_leave_pay = employee.hourly_rate * 8
    c.drawRightString(inch * 2, height - inch * 2.8, "Sick days:")
    c.drawString(inch * 3, height - inch * 2.8, f"{employee.sick_leave_days}days")
    c.drawString(inch * 4, height - inch * 2.8, f"{int(sick_leave_pay * 0.8)}kr")
    c.drawString(inch * 5, height - inch * 2.8, f"{employee.total_sick_pay}kr")
    ...
    c.drawString(
        inch * 2.1, height - inch * 6.8, f"Monthly tax:   {employee.total_tax}kr"
    )
    c.drawString(
        inch * 2.1, height - inch * 7.1, f"Monthly nett pay:   {employee.total_net}kr"
    )
    ...
    c.drawString(
        inch * 5, height - inch * 10.5, f"Yearly tax:{employee.total_year_tax}kr"
    )
